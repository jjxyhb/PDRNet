...
class Compose(object):
    ...
    def __call__(self, image, mask, mask_gt=None):
        for op in self.ops:
            image, mask, mask_gt = op(image, mask, mask_gt)
        return image, mask, mask_gt
...
class Resize(object):
    ...
    def __call__(self, image, mask, mask_gt=None):
        ...
        if mask_gt is not None:
            mask_gt = cv2.resize(mask_gt, dsize=(self.W, self.H), interpolation=cv2.INTER_LINEAR)
        return image, mask, mask_gt
...
class RandomCrop(object):
    ...
    def __call__(self, image, mask, mask_gt=None):
        ...
        if mask_gt is not None:
            mask_gt = mask_gt[ymin:ymin+self.H, xmin:xmin+self.W, :]
        return image, mask, mask_gt
...
class ToTensor(object):
    def __call__(self, image, mask, mask_gt=None):
        ...
        return image, mask.mean(dim=0, keepdim=True), mask_gt
